...
const onResizeHandleDoubleClick = toggleCollapsed;

const toggleIcon = computed(() => {
  const expandIcon = isRTL.value
    ? 'i-lucide-chevron-left'
    : 'i-lucide-chevron-right';
  const collapseIcon = isRTL.value
    ? 'i-lucide-chevron-right'
    : 'i-lucide-chevron-left';
  return isEffectivelyCollapsed.value ? expandIcon : collapseIcon;
});
  ...
  >
    ...
    >
      ...
      <button
        ...
        "
        class="absolute top-1/2 -translate-y-1/2 ltr:right-0 rtl:left-0 ltr:translate-x-1/2 rtl:-translate-x-1/2 flex items-center justify-center size-5 rounded-full border border-n-weak bg-n-solid-1 text-n-slate-11 shadow-sm opacity-70 hover:opacity-100 focus-visible:opacity-100 hover:bg-n-alpha-2 dark:hover:bg-n-slate-9/30 transition-opacity cursor-pointer"
        @mousedown.stop
        @click.stop="toggleCollapsed"
      >
        <span class="size-3" :class="toggleIcon" />
      </button>
    </div>
  </aside>
</template>
