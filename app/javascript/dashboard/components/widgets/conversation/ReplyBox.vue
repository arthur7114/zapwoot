...
import ScheduledMessagesQueue from './ScheduledMessagesQueue.vue';
...
export default {
  ...
  methods: {
    ...
    async onScheduleMessage({ content, scheduledAt }) {
      try {
        ...
        useAlert(this.$t('CONVERSATION.SCHEDULE_MESSAGE.SUCCESS_MESSAGE'));
        this.hideScheduleMessageModal();
        this.$refs.scheduledMessagesQueue?.refresh();
      } catch (error) {
        const errorMessage =
          error?.response?.data?.message ||
          error?.response?.data?.error ||
          this.$t('CONVERSATION.SCHEDULE_MESSAGE.ERROR_MESSAGE');
...
<template>
  ...
  <div ref="replyEditor" class="reply-box" :class="replyBoxClass">
    ...
    <ScheduledMessagesQueue
      v-if="!copilot.isActive.value && conversationId"
      ref="scheduledMessagesQueue"
      :conversation-id="conversationId"
    />

    <QuickReplies v-if="!copilot.isActive.value" :is-private="isPrivate" />
    ...
    <woot-modal
      ...
      :on-close="hideScheduleMessageModal"
    >
      <ScheduleMessageModal
